...
class PostController extends Controller
{
    public function index(Post $post, User $user)
    {
        ...
        $post = $post->whereDate('date','>=',$today)->orderBy('date', 'ASC')->paginate(10);
        // 非表示
        //掲載期限が近いものから順に表示
    ...
    public function show(Post $post, User $user, Comment $comment)
    {
        ...
        //dd($comment);
        return view('show')->with(['post' => $post, 'user' => $user, 'comments' => $comments, 'i' => $i]);
    }
    ...
    public function recuruit(Post $post, User $user)
    {
        ...
        $today = date("Y-m-d");
        $post = Post::where('user_id', $user['id'])->whereDate('date','>=',$today)->orderBy('date', 'ASC')->paginate(10);
        //$post = $user->post();
        //$posts = $user->posts();
        //dd($post);
        return view('recuruit')->with(['posts' => $post, 'today' => $today]);
    }
    ...
    public function recuruited(Post $post, User $user)
    {
        ...
        //$user = User::find($user->name);
        $today = date("Y-m-d");
        $post = Post::where('user_id', $user['id'])->whereDate('date','<',$today)->orderBy('date', 'DESC')->paginate(10);
        //$post = $user->post();
        //$posts = $user->posts();
        //dd($post);
    ...
    public function Participation(Post $post, User $user)
    {
        $today = date("Y-m-d");
        //$today = strtotime($today);
        $post = $post->whereDate('date','>=',$today)->orderBy('date', 'ASC')->paginate();
        // 非表示
        //掲載期限が近いものから順に表示
        // $post = $post->orderBy('updated_at', 'DESC')->paginate(10);
        //更新順に並び替え
        //dd($today);
        // $post = Post::where('date'<$today);
        return view('participation')->with(['posts' => $post, 'user' => $user, 'today' => $today]);
