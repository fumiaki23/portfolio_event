                    ...
                    <div class="row">
                        ...
                        <div class="col-4 text-right">
                            @if($post->users()->where('user_id', Auth::id())->exists())
                                ...
                                @csrf
                                   <input type="submit" value="&#xf004; いいね! {{ $post->users()->count() }}" class="fas btn text-danger mt-2 border">
                                </form>
                            @else
                                <form action="{{ route('favorites', $post) }}" method="POST">
                                @csrf
                                <input type="submit" value="&#xf004; いいね! {{ $post->users()->count() }}" class="fas btn mt-2 text-muted border">
                                </form>
                            @endif
                        </div>
                    </div>
